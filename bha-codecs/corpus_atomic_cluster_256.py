"""Cluster 5000 sequences into 256 buckets (2^8) with metadata vector.

Each bucket = 8-bit structural ID + metadata (offset, n_terms, first4,
sign_changes, monotonicity, ratio_signature, best_strategy).

256 buckets + metadata = 250+ families + service metadata.
Sparse buckets = unique patterns (need attention).
"""
from __future__ import annotations

import json
import sys
import time
from collections import Counter, defaultdict
from pathlib import Path

sys.path.insert(0, r"C:\Users\Art\.mimicode\skills\oeis-sequence-atomizer\scripts")
import oeis_sequence_atomizer as m


def structural_hash(first4: str, ratios: dict, n_terms: int, offset: int) -> int:
    """Compute 8-bit hash from first-4, ratio, n_terms, offset."""
    sig = (
        f"{first4}|"
        f"{min(ratios.values()):.0f}|{max(ratios.values()):.0f}|"
        f"{n_terms//10:x}|{offset:x}"
    )
    h = 0
    for c in sig:
        h = (h * 31 + ord(c)) & 0x7FFFFFFF
    return h % 256


def main() -> int:
    in_path = Path(r"D:\4\bha-codecs\benchmark\corpus-atomize-5000.json")
    full_path = Path(r"D:\4\bha-codecs\benchmark\corpus-atomize-5000.json")
    out_path = Path(r"D:\4\bha-codecs\benchmark\corpus-atomic-cluster-256.json")
    full_data = json.loads(full_path.read_text(encoding="utf-8"))
    results_dict = {}
    for entry in full_data["top_30_best"]:
        results_dict[entry["a_id"]] = entry
    for entry in full_data["top_30_worst"]:
        results_dict[entry["a_id"]] = entry
    for entry in full_data["top_30_variance"]:
        results_dict[entry["a_id"]] = entry
    print(f"input: {len(results_dict)} sequences (top30 + 30 + 30)")
    n_strat_dist = full_data.get("strategy_by_ratio_bucket", {})
    print(f"ratio distribution: {n_strat_dist}")

    # Build 256 buckets with metadata
    buckets = defaultdict(list)
    bucket_meta = defaultdict(lambda: {
        "first4_patterns": Counter(),
        "best_strategies": Counter(),
        "ratios": [],
        "variances": [],
        "n_terms": [],
        "offsets": [],
    })
    n_total = 0
    skipped = 0
    for a_id, r in results_dict.items():
        try:
            offset, n_terms, terms_str = m.fetch_term_string(a_id)
        except Exception:
            skipped += 1
            continue
        first4 = ",".join(terms_str.split(",")[:4])
        best_strat = r["best_strategy"]
        best_ratio = r["best_ratio"]
        variance = r["variance"]
        h = structural_hash(first4, {"best": best_ratio, "var": variance}, n_terms, offset)
        buckets[h].append(a_id)
        meta = bucket_meta[h]
        meta["first4_patterns"][first4] += 1
        meta["best_strategies"][best_strat] += 1
        meta["ratios"].append(best_ratio)
        meta["variances"].append(variance)
        meta["n_terms"].append(n_terms)
        meta["offsets"].append(offset)
        n_total += 1
    print(f"\nparsed: {n_total}, skipped: {skipped}")
    print(f"buckets used: {len(buckets)} of 256")

    # Density distribution
    sorted_buckets = sorted(buckets.items(), key=lambda kv: len(kv[1]), reverse=True)
    print(f"\n=== BUCKET DENSITY DISTRIBUTION ===")
    density_dist = Counter()
    for h, seqs in buckets.items():
        if len(seqs) >= 100:
            density_dist["100+"] += 1
        elif len(seqs) >= 50:
            density_dist["50-99"] += 1
        elif len(seqs) >= 20:
            density_dist["20-49"] += 1
        elif len(seqs) >= 10:
            density_dist["10-19"] += 1
        elif len(seqs) >= 5:
            density_dist["5-9"] += 1
        elif len(seqs) >= 2:
            density_dist["2-4"] += 1
        else:
            density_dist["1"] += 1
    def median_size(k, buckets):
        if k == "100+":
            return 100
        if k == "50-99":
            return 75
        if k == "20-49":
            return 35
        if k == "10-19":
            return 15
        if k == "5-9":
            return 7
        if k == "2-4":
            return 3
        return 1
    for k in ["100+", "50-99", "20-49", "10-19", "5-9", "2-4", "1"]:
        print(f"  {k}: {density_dist[k]} buckets, {density_dist[k] * median_size(k, buckets)} sequences")

    print(f"\n=== TOP 30 BUCKETS (dense) ===")
    print(f"{'bucket':>6} {'size':>5} {'first4':>20} {'avg_ratio':>10} {'var':>8}")
    for h, seqs in sorted_buckets[:30]:
        meta = bucket_meta[h]
        avg = sum(meta["ratios"]) / len(meta["ratios"])
        var = sum(meta["variances"]) / len(meta["variances"])
        first4 = meta["first4_patterns"].most_common(1)[0][0]
        print(f"{h:>6} {len(seqs):>5} {first4:>20} {avg:>10.2f} {var:>8.2f}")

    singletons = [(h, seqs) for h, seqs in buckets.items() if len(seqs) == 1]
    print(f"\n=== TOP 30 BUCKETS (singletons) ===")
    for h, seqs in singletons[:30]:
        meta = bucket_meta[h]
        first4 = meta["first4_patterns"].most_common(1)[0][0]
        avg = meta["ratios"][0]
        print(f"  bucket {h}: {seqs[0]} first4={first4} ratio={avg:.2f}%")

    # Save full per-bucket analysis
    out_buckets = {}
    for h, seqs in buckets.items():
        meta = bucket_meta[h]
        avg_r = sum(meta["ratios"]) / len(meta["ratios"])
        var = sum(meta["variances"]) / len(meta["variances"])
        avg_n = sum(meta["n_terms"]) / len(meta["n_terms"])
        out_buckets[str(h)] = {
            "size": len(seqs),
            "first4_top": meta["first4_patterns"].most_common(5),
            "best_strategies": dict(meta["best_strategies"]),
            "avg_ratio": round(avg_r, 2),
            "avg_variance": round(var, 2),
            "avg_n_terms": round(avg_n, 1),
            "sequences": seqs[:30],
            "singleton": len(seqs) == 1,
        }
    out = {
        "n_total": n_total,
        "n_skipped": skipped,
        "buckets_used": len(buckets),
        "buckets_total": 256,
        "density_distribution": dict(density_dist),
        "top_30_buckets": [{"bucket": h, **out_buckets[str(h)]} for h, _ in sorted_buckets[:30]],
        "singleton_buckets": [
            {"bucket": h, "id": seqs[0], "first4": bucket_meta[h]["first4_patterns"].most_common(1)[0][0],
             "ratio": round(bucket_meta[h]["ratios"][0], 2), "n_terms": bucket_meta[h]["n_terms"][0]}
            for h, seqs in singletons
        ],
        "all_buckets": out_buckets,
    }
    out_path.write_text(json.dumps(out, indent=2), encoding="utf-8")
    print(f"\nresults written to {out_path}")
    return 0


if __name__ == "__main__":
    sys.exit(main())
