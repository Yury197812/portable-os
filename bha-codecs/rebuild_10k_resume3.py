"""Finish the missing 2 files. Just append, don't redo what's already in OUT."""
...
CASES = [
    (r"D:\4\bha-codecs\benchmark\bro_specific_html_500k.html", 200),  # slow
    (r"D:\4\bha-codecs\benchmark\bro_html+json-80k.html", 5000),     # fast
]

for path, n in CASES:
    p = Path(path)
    if p.name in existing["files"]:
        print(f"skip {p.name}: already in OUT")
        continue
    data = p.read_bytes()
    sha = _sha256_file(p)
    sizes, pack_ms = [], []
    ...
    for i in range(n):
        ...
        if (i+1) % max(1, n//5) == 0:
            print(f"  [{i+1}/{n}] elapsed={time.perf_counter()-t0:.0f}s  last_ms={pm:.0f}  rt_fails={rt_fails}", flush=True)
    ...
    print(f"  >>> {p.name}: {n} in {elapsed:.0f}s  "
          f"median_pack={existing['files'][p.name]['pack_ms']['p50']:.1f}ms  "
          f"size_unique={len(set(sizes))}  rt_fails={rt_fails}", flush=True)

print(f"\nALL FILES: {list(existing['files'])}")
