"""Finish markdown-80k at 1000 iter only (it's slow)."""
...
p = Path(r"D:\4\bha-codecs\benchmark\bro_markdown-80k.md")
data = p.read_bytes()
sha = _sha256_file(p)
n = 1000
sizes, pack_ms = [], []
...
print(f"=== {p.name}  in={len(data):>10d}  n={n} ===", flush=True)
for i in range(n):
    t = time.perf_counter()
...
print(f"  >>> {p.name}: {n} in {elapsed:.0f}s  median_pack={existing['files'][p.name]['pack_ms']['p50']:.1f}ms  "
      f"p99={existing['files'][p.name]['pack_ms']['p99']:.1f}ms  size_unique={len(set(sizes))}  rt_fails={rt_fails}")
print(f"\nALL FILES IN REBUILD: {list(existing['files'])}")
